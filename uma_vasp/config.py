...
class Config:
    ...
    def __init__(self):
        self.api_token_client_id = self.require_env("LIGHTSPARK_API_TOKEN_CLIENT_ID")
        self.api_token_client_secret = self.require_env(
            "LIGHTSPARK_API_TOKEN_CLIENT_SECRET"
        )
        self.node_id = self.require_env("LIGHTSPARK_UMA_NODE_ID")
        self.encryption_pubkey_hex = self.require_env(
            "LIGHTSPARK_UMA_ENCRYPTION_PUBKEY"
        )
        self.encryption_privkey_hex = self.require_env(
            "LIGHTSPARK_UMA_ENCRYPTION_PRIVKEY"
        )
        self.signing_pubkey_hex = self.require_env("LIGHTSPARK_UMA_SIGNING_PUBKEY")
        self.signing_privkey_hex = self.require_env("LIGHTSPARK_UMA_SIGNING_PRIVKEY")
